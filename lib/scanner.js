...
class Scanner {
  ...
  async scanMigrationFiles() {
    // Wappler migration files (`.wappler/migrations/db/*.js`) register server
    // action file paths as data — either as object-literal `path:` values or
    // inside SQL `INSERT INTO ... VALUES(...)` string literals. Those actions
    // are invoked later by a runtime dispatcher that reads the DB rows, so
    // pure static code search never sees them as referenced. Scan migrations
    // for any string literal whose path matches one of our known action
    // files (handling both `/app/api/...` and `app/api/...` forms).
    const pattern = path.join(this.projectRoot, '.wappler/migrations/db/**/*.js');
    const files = await glob(pattern);

    if (files.length === 0) return;

    // Build a lookup of candidate action file-path strings (with and without
    // leading slash) → urlPath key used in `serverActions`.
    const pathLookup = new Map();
    for (const [urlPath, action] of this.serverActions) {
      const rel = action.relativePath.split(path.sep).join('/'); // normalize win sep
      pathLookup.set(rel, urlPath);                // e.g. app/api/v1/queues/...
      pathLookup.set('/' + rel, urlPath);          // e.g. /app/api/v1/queues/...
    }

    // String-literal regex: matches single-, double-, or back-tick-quoted
    // strings whose contents look like an action file path ending in `.json`.
    const literalRegex = /["'`]\/?app\/(?:api|lib)\/[^"'`]+?\.json["'`]/g;
    ...
    for (const file of files) {
      ...
      const matches = content.match(literalRegex);
      if (!matches) continue;

      for (const raw of matches) {
        // Strip surrounding quote char
        const inner = raw.slice(1, -1);
        const urlPath = pathLookup.get(inner);
        if (!urlPath) continue;

        const action = this.serverActions.get(urlPath);
        if (!action) continue;

        const sourceRel = path.relative(this.projectRoot, file);
        action.references.push({
          sourceFile: sourceRel,
          type: 'migration-path',
          originalReference: inner,
        });

        if (!this.references.has(urlPath)) {
          this.references.set(urlPath, []);
        }
        this.references.get(urlPath).push({
          sourceFile: sourceRel,
          type: 'migration-path',
          originalReference: inner,
        });
  ...
  analyzeResults() {
    ...
    for (const [urlPath, action] of this.serverActions) {
      ...
      // Queue modules (`app/api/v1/queues/**/modules/**`) and flag actions
      // (`app/api/v1/queues/flags/actions/**`) are registered in the database
      // (often via an ancient seed or a since-squashed migration), then
      // invoked at runtime by a dispatcher that reads the DB rows. A clean
      // static scan can miss them entirely, so a zero-reference verdict here
      // is NOT safe to act on. Flag them for review instead.
      const rel = (action.relativePath || '').split(path.sep).join('/');
      const isRuntimeDispatched =
        /^app\/api\/v1\/queues\/[^/]+\/modules\//.test(rel) ||
        rel.startsWith('app/api/v1/queues/flags/actions/');

      // Simplified confidence system: only "safe to delete" vs "review needed"
      if (referenceCount === 0) {
        if (isRuntimeDispatched) {
          confidence = 'review-needed';
          status = 'unreferenced-queue-module';
          results.summary.possiblyUnused++;
        } else {
